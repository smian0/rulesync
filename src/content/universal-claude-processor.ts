...
export class UniversalClaudeProcessor extends FeatureProcessor<RulesyncFile, ToolFile> {
  ...
  static getToolTargets(): ToolTarget[] {
    return ["cursor", "opencode", "codexcli"];
  }
      ...
      try {
        ...
        if (this.toolTarget === "cursor") {
          ...
          if (contentType === "subagents") {
            ...
            // Route CCMP rules to .opencode/rules/ (skip standard rulesync overview.md)
            const OpencodeContentContent = this.createOpencodeContentContent();
            toolFile = new OpencodeContentContent({
              fileName: cleanFileName,
              fileContent: body,
              contentType: contentType,
              relativePath: originalPath,
            ...
            // Route any non-standard content types (custom directories) to flat .opencode/{contentType}/
            const OpencodeContentContent = this.createOpencodeContentContent();
            toolFile = new OpencodeContentContent({
              fileName: cleanFileName,
              fileContent: body,
        ...
        } else if (this.toolTarget === "codexcli") {
          // Route all content to .codex/memories/ using consistent naming
          const CodexcliContent = this.createCodexcliContent();
          toolFile = new CodexcliContent({
            fileName: cleanFileName,
            fileContent: body,
            contentType: contentType,
            relativePath: originalPath,
  ...
  private createOpencodeContentContent() {
    return class OpencodeContentContent extends ToolFile {
      private readonly contentType: string;
      private readonly sourceRelativePath: string;
      ...
      constructor({ fileName, fileContent, contentType, relativePath, directoryPath }: {
        ...
        const baseName = fileName.replace(/\.[^/.]+$/, ""); // Remove extension
        const contentFileName = `${baseName}.md`;
        
        super({
          baseDir: ".",
          relativeDirPath: `.opencode/${contentType}`,  // Flat directory structure
          relativeFilePath: contentFileName,
          fileContent: fileContent,
          validate: false
        });
      ...
      getFileContent(): string {
        let originalContent = super.getFileContent();
        
        // Implement path rewriting for commands that reference .claude/ directories
        if (this.contentType === "commands") {
          // Replace .claude/ references with relative paths in .opencode
          originalContent = originalContent
            .replace(/\.claude\/context\//g, "./context/")
            .replace(/\.claude\/epics\//g, "./epics/")
            .replace(/\.claude\/prds\//g, "./prds/")
            .replace(/\.claude\/rules\//g, "./rules/")
            .replace(/\.claude\/technical\//g, "./technical/")
            // Also handle /rules/ references (absolute paths within claude structure)
    ...
    return class OpencodeCommandContent extends ToolFile {
      ...
      getFileContent(): string {
        ...
        if (frontmatterMatch) {
          ...
          // Apply path rewriting for .claude/ references
          body = body
            .replace(/\.claude\/context\//g, "./context/")
            .replace(/\.claude\/epics\//g, "./epics/")
            .replace(/\.claude\/prds\//g, "./prds/")
            .replace(/\.claude\/rules\//g, "./rules/")
            .replace(/\.claude\/technical\//g, "./technical/")
            .replace(/\.claude\/scripts\//g, "./scripts/")
            .replace(/\/rules\//g, "./rules/");
...
${body.trim()}`;
  ...
  private createCodexcliContent() {
    return class CodexcliContent extends ToolFile {
      private readonly contentType: string;
      private readonly sourceRelativePath: string;
      ...
      constructor({ fileName, fileContent, contentType, relativePath, directoryPath }: {
        ...
        const baseName = fileName.replace(/\.[^/.]+$/, ""); // Remove extension
        const codexFileName = `${baseName}.md`;
        
        super({
          baseDir: ".",
          relativeDirPath: ".codex/memories",
          relativeFilePath: codexFileName,
          fileContent: fileContent,
          validate: false
        });
      ...
      getFileContent(): string {
        ...
        // Apply path rewriting for .claude/ references (similar to OpenCode)
        if (this.contentType === "commands") {
          originalContent = originalContent
            .replace(/\.claude\/context\//g, "./.codex/memories/")
            .replace(/\.claude\/epics\//g, "./.codex/memories/")
            .replace(/\.claude\/prds\//g, "./.codex/memories/")
            .replace(/\.claude\/rules\//g, "./.codex/memories/")
            .replace(/\.claude\/technical\//g, "./.codex/memories/")
            .replace(/\.claude\/scripts\//g, "./.codex/memories/")
            .replace(/\/rules\//g, "./.codex/memories/");
        }
        
        // Format content for Codex CLI with metadata header
        const header = `<!-- Generated from .claude/${this.contentType}/${this.sourceRelativePath} -->\n` +
                      `<!-- Content Type: ${this.contentType} -->\n` +
                      `<!-- Generated by rulesync Universal Claude Code Sync -->\n`;
